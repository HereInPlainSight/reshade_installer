...
ReShadeDlls="$ReShadeHome/dataDirs/dlls"
ReShadeInstallerRepo="https://www.github.com/HereInPlainSight/reshade_installer"
...
fetchCompilers(){
  if [ ! -d "$ReShadeDlls/d3dcompiler_47s" ]; then
    mkdir -p "$ReShadeDlls/d3dcompiler_47s"
  fi
  pushd "$ReShadeDlls/d3dcompiler_47s" > /dev/null || exit
...
updateInstalls(){
  updating="$1"
  if [[ "$2" != "skip" ]]; then performBackup; fi
  while IFS="=;" read -r gameName installDir prefixDir; do
...
##
# Update include.d / blacklist.d directories from upstream.  Ensure the Off.ini is up to date so long as we're there.
etCloneHome(){
  gitHome="$ReShadeHome/dataDirs/git"
  if [ ! -d "$gitHome" ]; then mkdir -p "$gitHome"; fi
  pushd "$gitHome" > /dev/null || exit
  gitMaint "$ReShadeInstallerRepo" "reshade_installer"
  ln -sfn "$gitHome/reshade_installer/reshade_installer.sh" "$ReShadeHome/reshade_installer.sh"
  homeRepo="$gitHome/reshade_installer"
  if [ ! -d "$ReShadeHome/include.d" ]; then mkdir -p "$ReShadeHome/include.d"; fi
  cp "$homeRepo/include.d/*" "$ReShadeHome/include.d/"
  if [ ! -d "$ReShadeHome/blacklist.d" ]; then mkdir -p "$ReShadeHome/blacklist.d"; fi
  cp "$homeRepo/blacklist.d/*" "$ReShadeHome/blacklist.d/"
  if [ ! -d "$ReShadeHome/presets"]; then mkdir -p "$ReShadeHome/presets"; fi
  cp "$homeRepo/Off.ini" "$ReShadeHome/dataDirs/presets/Off.ini"
}
...
cleanSources(){
  ...
  find "$(pwd)" -mindepth 1 -maxdepth 1 -type d $(printf "! -name %s " $(find . -type d -name '.git' -print | cut -c 3- | cut -f1 -d"/" | uniq)) -exec rm -r {} \; 2>&1 | grep -v "find: paths must precede expression: "
}
...
runSources(){
  ...
  for i in "${!sourceList[@]}"; do
	...
	directoryName="$directoryName-$counter"
      ...
      elif [[ "${addr[2]}.${addr[3]}" == "gitlab.com" ]]; then
      	sourceList[$i]="${addr[0]}//gitlab.com/${addr[4]}/${addr[5]}"
	gitMaint "${sourceList[$i]}" "${addr[4]}/${addr[5]}"
	returnList+=("${addr[4]}"/"${addr[5]}")
      elif [[ "${addr[2]}" == "" ]]; then :	# : is null in bash.  We skip empty lines.
      else
        # Print out the address structure if something failed.
	printf "Failure to import!  Please report this issue with all the below data if you believe this is in error:\n"
        for j in "${!addr[@]}"; do
          printf "$j:\t${addr[$j]}\n"
        done
        printf "\n"
...
# Requires data in 'include.d/*.preset' file(s).
presetsUpdate(){
...
update(){
  if [ ! -f "$ReShadeHome/version" ]; then
    ...
    if [ "$mia" = "7z" ] && ( hash "ark" &>/dev/null ); then
      if ( yesNo "7z not found -- would you like to download a binary copy of the 7zip extractor?  This should ONLY be used if your package manager does not offer 7zip, such as the Steam Deck!  " ); then
        if [ ! -d "$ReShadeDlls/d3dcompiler_47s/7z" ]; then
          mkdir -p "$ReShadeDlls/d3dcompiler_47s/7z"
        fi
        pushd "$ReShadeDlls/d3dcompiler_47s/7z" > /dev/null || exit
        printf "\nWorking..."
        zipVer="16.02"
    ...
    mkdir -p "$ReShadeHome" && pushd "$_" > /dev/null && touch games.db && mkdir -p "dataDirs/presets/Custom" && popd > /dev/null || exit
    ##
    # Clone the repo, absorb the include.d and blacklist.d directories.  Pull the Off.ini file and the ReShade.ini file.  (Burn out the curl from above after completion.)
    etCloneHome
    fetchCompilers
  fi
  ...
  if [ "$forceUpdate" -eq 0 ] && [[ -f "$ReShadeHome/version" ]] && [[ $(<"$ReShadeHome/version") == "$reshadeCurrent" ]] && [[ -f "$ReShadeHome/ReShade64.dll" ]]; then
    ...
    if [[ ! -f "$ReShadeHome/ReShade.ini" ]]; then
      ...
      curl -sLO https://raw.githubusercontent.com/HereInPlainSight/reshade_installer/reshade/ReShade.ini
    fi
    if [[ -f "$ReShadeDlls/ReShade/ReShade64.dll" ]]; then
      pushd "$ReShadeDlls/ReShade" > /dev/null || exit
      printf "\e[2K\rmd5sums in process..."
      old64="$(getMD5 ReShade64.dll)"
      old32="$(getMD5 ReShade32.dll)"
      popd > /dev/null || exit
    else
      mkdir -p "$ReShadeDlls/ReShade"
    fi
    shadersUpdate
    presetsUpdate
    printf "\e[2K\rDownloading latest ReShade...                     "
    curl -sLO https://reshade.me/downloads/ReShade_Setup_$reshadeCurrent\_Addon.exe
    unzip -qqo ReShade_Setup_$reshadeCurrent\_Addon.exe -d "$ReShadeDlls/ReShade" &> /dev/null
    if [[ -f "$ReShadeHome/version" ]] && [[ -f "$ReShadeHome/ReShade_Setup_$(<$ReShadeHome/version)_Addon.exe" ]]; then rm -rf $(echo "$ReShadeHome/ReShade_Setup_$(<$ReShadeHome/version)_Addon.exe"); fi
    printf "%s\n" "$reshadeCurrent" > version
    updateInstalls all skip
    popd > /dev/null || exit
    printf "\e[2K\rReShade-%s installed.\n" "$reshadeCurrent"
  fi
}
...
listGames(){
  ...
  while IFS="=;" read -r gameName installDir prefixDir; do
    ...
    gapiln=$(find '.' -maxdepth 1 -name "*.dll" -lname "$ReShadeDlls/ReShade/ReShade*.dll" -exec basename {} ';')
    if [ -z "$gapiln" ]; then
      fileString="$(file "$gameName")"
      gmd5=""
      if printf "%s" "$fileString" | grep -q "80386"; then
        gmd5="$(getMD5 "$ReShadeDlls/ReShade/ReShade32.dll")"
      elif printf "%s" "$fileString" | grep -q "x86-64"; then
        gmd5="$(getMD5 "$ReShadeDlls/ReShade/ReShade64.dll")"
      fi
...
#  WINEPREFIX="${WINEPREFIX//+(\/)//}"		# Legacy, but interesting to remember.
  ...
  ln -sfn "$ReShadeDlls/d3dcompiler_47s/d3dcompiler_47.dll.${ARCH}bit" d3dcompiler_47.dll
  if [ $? != 0 ] || [ ! -L "d3dcompiler_47.dll" ]; then cp "$ReShadeDlls/d3dcompiler_47s/d3dcompiler_47.dll.${ARCH}bit" d3dcompiler_47.dll; fi
  ln -sfn "${ReShadeDlls}/ReShade/ReShade${ARCH}.dll" "${gapi}".dll
  ...
  if [ -d "$gameLoc/reshade/presets" ]; then
    ...
    if ! cmp -s "$ReShadeHome/ReShade.ini" "ReShade.ini"; then cp -a "ReShade.ini" "$backupDir"; fi
...
debugInfo(){
  ...
  pushd "$ReShadeHome" > /dev/null || exit
  printf "Checking md5sums..."
  md3d32=$(getMD5 "$ReShadeDlls/d3dcompiler_47s/d3dcompiler_47.dll.32bit")
  ##
...
#    PS=2
  ...
  if [ "$(getMD5 $ReShadeDlls/d3dcompiler_47s/d3dcompiler_47.dll.64bit)" == "b0ae3aa9dd1ebd60bdf51cb94834cd04" ]; then true; else false; fi
  N64=$?
  output=$(printf "%b" "\e[2K\rInstallation location:\t${ReShadeHome/#$HOME/"\$HOME"}/
Installation version:\t$(cat "$ReShadeHome"/version)
$([ "$IS_MAC" = true ] && printf "%b" "Environment:\t\t\e[33mMac OS detected" || printf "d3dcompiler_47 32-bit:\t$([ "$PS" -eq 0 ] && printf "\e[32mOK" || printf "%b" "\e[31mmd5sum failure")$([ "$PS" -eq 2 ] && printf "%50s" " \e[33mLegacy file -- please run 'fetchCompilers'!") "%15s"\e[0m "ReShade32.dll:" $([ -f "$ReShadeDlls"/ReShade/ReShade32.dll ] && printf "\e[32mExists" || printf "%b" "\e[31mMIA")")\e[0m
d3dcompiler_47 64-bit:	$([ "$N64" -eq 0 ] && printf "\e[32mOK" || printf "%b" "\e[31mmd5sum failure")                \e[0m "ReShade64.dll:" $([ -f "$ReShadeDlls"/ReShade/ReShade64.dll ] && printf "\e[32mExists" || printf "\e[31mMIA")\e[0m")
